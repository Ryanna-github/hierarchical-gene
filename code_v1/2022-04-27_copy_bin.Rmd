...
K_up <- 4
# 第二次循环，后每次循环代码相同
tau <- 1 # 拉格朗日乘子，惩罚参数，对应原代码 penal.parac
# mu <- fmr$mu_k_weight[fmr$ci_weight]
# rho <- fmr$rho_k_weight[fmr$ci_weight]
# q_c_matrix <- matrix(0, nrow = n, ncol = K_up)
# for(k in 1:K_up){
#   q_c_matrix[,k] <- dnorm(whole.data$data_y, mean = fmr$mu_k_weight[k], sd = 1/fmr$rho_k_weight[k])
# }
# q_c_matrix <- q_c_matrix * kronecker(matrix(1, nrow = n, ncol = 1), pi_k)
