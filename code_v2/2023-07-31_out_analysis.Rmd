...
path_pre <- glue("{prefix}{Sys.Date()}_real2_n{n}_p{p}_q{q}_eps{epsilon_sd}_Kup{K_up}_fminit")
# path_pre <- glue("2023-07-31_n{n}_p{p}_q{q}_bl{beta_vlen}_al{alpha_vlen}_eps{epsilon_sd}_fminit")
...
df <- read.csv("2024-03-06_n500_p80_q40_b1_ss2_bl3_al2_eps0.5_epi0.5_rr0.1_Kup6_ctEn_fminit.csv")
# 以下方法过期，直接看 tag 列即可
...
df_report <- df %>% 
  select(dt_seed, q_c_seed, l2, l3, mse, 
         ...
         main_grn, sub_grn, fit_mean, penal, bic_sum, bic_mean, tag) %>%
  mutate(bic_mean2 = fit_mean + penal/log(n)) %>%
  filter(l2 > 0 & l3 > 0) %>%
  filter(tag == 'hier') %>%
