...
df <- read.csv("2023-11-27_n500_p8_q4_bl3_al2_eps0.5_Kup4_fminit.csv")
...
t <- df_new %>% 
  group_by(dt_seed) %>%
  summarise(count = n(),
            K_main_correct_num = sum(main_grn == 2),
            K_sub_correct_num = sum(sub_grn == 4),
            K_all_correct_num = sum(sub_grn == 4 & main_grn == 2)) %>%
  mutate(K_main_correct_ratio = K_main_correct_num/count,
         K_sub_correct_ratio = K_sub_correct_num/count,
         K_all_correct_ratio = K_all_correct_num/count)
```
...
```{r}
ggplot(df_new, aes(x = l2, y = l3)) +
  geom_point(alpha = 0.002, aes(size = sc))
```
```{r}
df_new %>% group_by(l2, l3) %>%
  summarise(count = n(), mse = mean(mse), sc = mean(sc),
            k_main = mean(main_grn), k_sub = mean(sub_grn)) %>%
  arrange(-sc)
```
