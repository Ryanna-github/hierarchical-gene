...
for(dt_seed in seq(9, 49, 10)){
    path_pre <- glue("hierarchical-gene/code_v2/{Sys.Date()}_n{n}_p{p}_q{q}_bl{beta_vlen}_al{alpha_vlen}_eps{epsilon_sd}_Kup{K_up}_fminit")
  ...
  cmd <- glue("Rscript --verbose test.R -n {n} --dt_seed {dt_seed} -p {p} -q {q} -e {epsilon_sd} --beta_vlen {beta_vlen} --alpha_vlen {alpha_vlen} --K_up {K_up} --path {path_pre}.csv >> {path_pre}_log.out")
  print(cmd)
}
```
