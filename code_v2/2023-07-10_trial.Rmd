...
```{r}
# plot(res$q_c_seed, res$ci_prob_mean)
plot(res$lambda_1, res$mse)
plot(res$lambda_2, res$mse)
plot(res$lambda_3, res$mse)

plot(res$lambda_1, res$cdist)
plot(res$lambda_2, res$cdist)
plot(res$lambda_3, res$cdist)
```
...
coef_full_init <- coef$coef_full - coef$coef_full 
set.seed(999)
# coef_full_init[(p+1):(p+q),] <- coef$coef_alpha + rnorm(q*K_up, 0, 2)
# coef_full_init[1:p,] <- coef$coef_beta + rnorm(p*K_up, 0, 2)
# coef_full_init <- coef$coef_full * t(kronecker(rho_init, matrix(1, ncol = p+q)))
coef_full_list <- list(coef_full_init)
coef_full_ori_list <- list(coef_full_init/t(kronecker(rho_init, 
                                                      matrix(1, ncol = p+q))))
