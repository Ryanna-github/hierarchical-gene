...
p1 <- ggplot(df_heat_melt, aes(x=level, y=id, fill=cl)) +
  ...
  xlab("")
p1
ggsave("heat_fev1.pdf", p1, width = 8, height = 10, units = "cm")
```
...
```{r}
trail_fev1$ci_est_sub
```

```{r}
sc(trail$ci_est_sub, trail_fev1$ci_est_sub)
sc(trail$ci_est_main, trail_fev1$ci_est_main)
```
```{r}
library(pheatmap)
...
```
```{r}
plot(rowSums(abs(trail_fev1$coef_full_ori_comp[7:36,])),
     rowSums(abs(trail_dlco$coef_full_ori_comp[7:36,])))
```
