...
```{r}
FMR2_comp <- function(q_c_seed, minprior_value = 0.2, tag = "FMR2"){
  set.seed(q_c_seed)
  pi_init <- rep(1/K_up, K_up)
  rho_init <- rep(1, K_up)/sigma_est
  q_c_matrix <- abs(t(kronecker(pi_init, matrix(1, ncol = n))) + rnorm(n*K_up, mean = 0, sd = .1))
...
respclust2_comp <- function(q_c_seed, tag = "respclust2"){
  ...
  idx_chosen <- which.max(rowMeans(df_K_set[,3:6], na.rm = TRUE))
...
```{r}
# df <- read.csv("2024-03-10_n500_p80_q40_b1_ss1_bl3_al2_eps0.5_epi0.5_rr0_Kup4_ctEn_oracle.csv")
df <- read.csv("2024-03-11_n500_p80_q40_b3_ss2_bl3_al2_eps0.5_epi0.5_rr0_Kup4_ctEn_oracle.csv")
df_result <- df %>% 
  filter(est_sub_grn >= 4) %>%
  group_by(dt_seed) %>%
...
```{r}
para_set <- expand.grid(list(balance = c(1,2,3),
                 signal_size = c(1,2),
                 K_up = c(4,6)))
# para_set <- expand.grid(list(balance = c(2,3),
#                  signal_size = c(1,2),
#                  K_up = c(4,6)))
```
...
for(i in 1:nrow(para_set)){
  balance <- para_set$balance[i]
  signal_size <- para_set$signal_size[i]
  K_up <- para_set$K_up[i]
    ...
    for(q_c_seed in 1:q_c_seed_max){
      FMR1_res <- tryCatch({ data.frame(FMR1_comp(q_c_seed)) }, 
               error = function(err) { cat("Error occurred when:", dt_seed, 
                                           "q_c_seed:", q_c_seed, "\n") ; NULL})
      FMR2_res <- tryCatch({ data.frame(FMR2_comp(q_c_seed, minprior_value = 0.15)) }, 
               error = function(err) { cat("Error occurred when:", dt_seed, 
                                           "q_c_seed:", q_c_seed, "\n") ; NULL})
  ...
  comresult_mean <- comresult %>% group_by(tag, dt_seed) %>%
    summarise(tag = min(tag),
              dt_seed = min(dt_seed),
              per_main = sum(est_main_grn == 2, na.rm = FALSE)/n(),
              per_sub = sum(est_sub_grn == 4, na.rm = FALSE)/n(),
              K_main = mean(est_main_grn, na.rm = FALSE),
              K_sub = mean(est_sub_grn, na.rm = FALSE),
  ...
  filename <- glue("comp_n{n}_p{p}_q{q}_b{balance}_ss{signal_size}_bl{beta_vlen}_al{alpha_vlen}_eps{epsilon_sd}_epi{epsilon_sd_init}_Kup{K_up}.csv")
  write.csv(comresult_mean, file = filename, row.names = FALSE)
  print(filename)
}
```
...
df_dict <- read.csv("summary_data/filedict.csv")
dim_type <- ifelse(p == 8, "low", "high")
path_prop <- df_dict[(df_dict$dim == dim_type) & (df_dict$balance == balance) &
                       (df_dict$signal == signal_size) & (df_dict$K_up == K_up),]$file
# 1. 读取各部分数据
# df_prop <- read.csv(glue("2024-02-26_n500_p{p}_q{q}_ss{signal_size}_bl{beta_vlen}_al{alpha_vlen}_eps0.5_epi0.5_rr0.2_Kup{K_up}_fminit.csv"))
# df_prop <- read.csv(glue("2024-02-27_n500_p{p}_q{q}_b{balance}_ss{signal_size}_bl{beta_vlen}_al{alpha_vlen}_eps0.5_epi0.5_rr0.2_Kup{K_up}_CtEn_fminit.csv"))

df_prop <- read.csv(path_prop)
df_comp <- read.csv(glue("comp_n{n}_p{p}_q{q}_b{balance}_ss{signal_size}_bl{beta_vlen}_al{alpha_vlen}_eps{epsilon_sd}_epi{epsilon_sd_init}_Kup{K_up}.csv"))
if(p == 8){
  df_orac <- read.csv(glue("2024-03-26_n500_p{p}_q{q}_b{balance}_ss{signal_size}_bl3_al2_eps0.5_epi0.5_rr0_Kup4_ctEn_oracle.csv"))
}else{
  df_orac <- read.csv(glue("2024-03-27_n500_p{p}_q{q}_b{balance}_ss{signal_size}_bl3_al2_eps0.5_epi0.5_rr0_Kup4_ctEn_oracle.csv"))
}

colnames_sum <- c('tag','dt_seed','per_main','per_sub',
                  'K_main', 'K_sub', 
                  'sc_main','sc_sub',
                  'ari_main','ari_sub',
                  'MSE_main','MSE_sub')

# 2. 各部分进行处理并拼接（to merge）
# 2.1 propose 方法
df_prop_tm <- df_prop %>% 
  filter(l2 > 0 & l3 > 0 & tag == 'hier') %>%
  select(dt_seed, q_c_seed, l2, l3, cdist_main, cdist_sub, sc_main, sc_sub, 
         ari_main, ari_sub, main_grn, sub_grn, fit_mean, penal, bic_sum, bic_mean, tag) %>%
  group_by(dt_seed, q_c_seed, tag) %>%
  # group_by(dt_seed, tag) %>% 
  arrange(bic_mean) %>%
  filter(row_number() == 4) %>%
  arrange(dt_seed, q_c_seed) # 检查节点（此时为每个 dt_seed q_c_seed 组合的最佳结果）
df_prop_tm <- df_prop_tm %>%
  group_by(dt_seed) %>%
  summarise(
            # per_main = as.numeric(main_grn == 2),
            # per_sub = as.numeric(sub_grn == 4),
            per_main = sum(main_grn == 2)/n(),
            per_sub = sum(sub_grn == 4)/n(),
            K_main = mean(main_grn),
            K_sub = mean(sub_grn),
...
df_prop_tm$tag <- "proposed"
df_prop_tm <- df_prop_tm %>% select(colnames_sum)

## 2.2 oracle 方法
df_orac_tm <- df_orac %>% 
  rename(l2 = lambda_2, l3 = lambda_3, 
         sc_main = sc_score_main, sc_sub = sc_score_sub,
         ari_main = ari_score_main, ari_sub = ari_score_sub,
         main_grn = est_main_grn, sub_grn = est_sub_grn) %>%
  select(dt_seed, q_c_seed, l2, l3, 
         cdist_main, cdist_sub, sc_main, sc_sub, 
         ari_main, ari_sub, main_grn, sub_grn, bic_sum, bic_mean, tag) %>%
  group_by(dt_seed, q_c_seed, tag) %>%
  # group_by(dt_seed, tag) %>%
  arrange(bic_mean) %>%
  filter(row_number() == 1) %>%
  arrange(dt_seed, q_c_seed) # 检查节点（此时为每个 dt_seed q_c_seed 组合的最佳结果）
df_orac_tm <- df_orac_tm %>%
  group_by(dt_seed) %>%
  summarise(
            # per_main = as.numeric(main_grn == 2),
            # per_sub = as.numeric(sub_grn == 4),
            ...
            MSE_main = mean(cdist_main, na.rm = T),
            MSE_sub = mean(cdist_sub, na.rm = T))
df_orac_tm$tag <- "oracle"
df_orac_tm <- df_orac_tm %>% select(colnames_sum)

## 2.3 compare 方法（保存结果已为整理后结果）
df_comp_tm <- df_comp

# 3. 整合结果，进行绘图
df_plot <- rbind(df_prop_tm, df_orac_tm, df_comp_tm) %>% select(-'dt_seed')
df_plot_long <- melt(df_plot, id.vars = "tag")
df_plot_long$tag <- factor(df_plot_long$tag, levels = c('proposed', 'oracle', 'FMR2', 'resiclust2', 'respclust2', 'FMR1', 'resiclust1', 'respclust1'))


score_names <- c("K_main", "K_sub", "sc_main", "sc_sub", "MSE_main", "MSE_sub")
df_tb <- NULL
for(i in 1:length(score_names)){
  col <- rlang::sym(score_names[i])
  df_tb_cur <- df_plot %>% group_by(tag) %>% 
    mean_table(!!col, digits = 4) %>%
    mean_format("mean (sd)", digits = 4) %>%
    select(response_var, group_cat, formatted_stats) %>%
    rename(tag = group_cat)
  df_tb <- rbind(df_tb, df_tb_cur)
}

df1 <- df_plot %>% group_by(tag) %>%
  summarise(per_main = mean(per_main, na.rm = FALSE),
            per_sub = mean(per_sub, na.rm = FALSE))
df2 <- dcast(df_tb, tag~response_var)
df_tb <- merge(df1, df2, by = 'tag')


df_tb$tag <- factor(df_tb$tag, levels = c("proposed", "oracle", "FMR2", "resiclust2", "respclust2", "FMR1", "resiclust1", "respclust1"))
df_tb <- df_tb %>% arrange(tag) %>% select(tag, per_main, K_main, sc_main, MSE_main,
                                           per_sub, K_sub, sc_sub, MSE_sub)
df_tb 
write.table(df_tb, "clipboard", sep="\t", row.names=FALSE)
```



```{r}
# 重新画图，思路和以前一样，但是重新修改变量
get_btype_plot <- function(ss, K_up_plot, btype){
  df_use <- df %>% filter(bl == btype & signal == ss & K_up == K_up_plot)
  p_list <- NULL
  score_names <- c("per_main", "per_sub", "K_main", "K_sub", 
                   "SC_main", "SC_sub", "MSE_main", "MSE_sub")
  for(i in 1:length(score_names)){
    col <- rlang::sym(score_names[i])
...
```{r}
library(tidyverse)
# df <- read.csv("summary_data/lowxz_b23_mod.csv")
df <- read.csv("summary_data/highxz_b23_mod.csv")

df$method <- factor(df$method, levels = c("proposed", "oracle", "FMR2", "resiclust2","respclust2", "FMR1", "resiclust1", "respclust1"))
df <- df %>%
  separate(SC_main, into = c("SC_main", "SC_main_std"), sep = "\\(", convert = TRUE) %>%
  separate(SC_sub, into = c("SC_sub", "SC_sub_std"), sep = "\\(", convert = TRUE) %>%
  separate(K_main, into = c("K_main", "K_main_std"), sep = "\\(", convert = TRUE) %>%
  separate(K_sub, into = c("K_sub", "K_sub_std"), sep = "\\(", convert = TRUE) %>%
  separate(MSE_main, into = c("MSE_main", "MSE_main_std"), sep = "\\(", convert = TRUE) %>%
  separate(MSE_sub, into = c("MSE_sub", "MSE_sub_std"), sep = "\\(", convert = TRUE) %>%
  mutate(SC_main_std = str_sub(SC_main_std, end = -2),
         SC_sub_std = str_sub(SC_sub_std, end = -2),
         K_main_std = str_sub(K_main_std, end = -2),
         K_sub_std = str_sub(K_sub_std, end = -2),
         MSE_main_std = str_sub(MSE_main_std, end = -2),
         MSE_sub_std = str_sub(MSE_sub_std, end = -2)) %>%
...
ss <- 1
K_up_plot <- 4
p1_list <- get_btype_plot(ss, K_up_plot,1)
p2_list <- get_btype_plot(ss, K_up_plot,2)
p3_list <- get_btype_plot(ss, K_up_plot,3)
p_list <- c(p1_list, p2_list, p3_list)
```

```{r}
pdf(glue('test_ss{ss}_Kup{K_up_plot}.pdf'), height = 10, width = 14)
do.call("ggarrange", c(p_list, nrow = 3, ncol = 8, common.legend = TRUE, legend = "bottom"))
dev.off()
```
...
```{r}
get_plot <- function(df_plot_long){
  p_list <- NULL
  # score_names <- c("MSE_main", "MSE_sub", "per_main", "per_sub", "sc_main", "sc_sub")
  score_names <- c("per_main", "per_sub", "sc_main", "sc_sub", "MSE_main", "MSE_sub")
  for(i in 1:length(score_names)){
    df_use <- df_plot_long %>% filter(variable == score_names[i])
    # if(score_names[i] %in% c("per_main", "per_sub", "sc_main", "sc_sub")){
    #   df_use$value[is.na(df_use$value)] <- 0
    # }
    # df_use$value[is.na(df_use$value)] <- -1
    p_list[[i]] <- ggplot(df_use, aes(x = tag, y = value, col = tag)) +
      geom_boxplot(na.rm = FALSE) +
      # scale_fill_brewer(palette = "Pastel1") +
      scale_fill_manual(values = c("proposed" = colors[1],
                                   "oracle" = colors[2],
                                   "FMR2" = colors[3],
                                   "resiclust2" = colors[4],
                                   "respclust2" = colors[5],
...
p_list <- get_plot(df_plot_long)
do.call("ggarrange", c(p_list, nrow = 3, ncol = 2, common.legend = TRUE, legend = "bottom"))
```
...
# 实验阶段版本
# mean
# 现在不同 q_c_seed 中找最好结果（实际因为没有评价准则 都用平均）
comresult_mean <- comresult %>% group_by(tag, dt_seed) %>%
  summarise(tag = min(tag),
            dt_seed = min(dt_seed),
            per_main = sum(est_main_grn == 2, na.rm = FALSE)/n(),
            ...
            MSE_main = mean(cdist_main, na.rm = TRUE),
            MSE_sub = mean(cdist_sub, na.rm = TRUE), .groups = "drop")
# comresult_mean2 <- comresult_mean %>% group_by(tag) %>%
#   summarise(tag = min(tag),
...
# std
comresult_std <- comresult %>% group_by(tag, dt_seed) %>%
  summarise(tag = min(tag),
            dt_seed = min(dt_seed),
            per_main = sd(est_main_grn == 2, na.rm = FALSE),
            per_sub = sd(est_sub_grn == 4, na.rm = FALSE),
            sc_main = sd(sc_score_main, na.rm = TRUE),
            sc_sub = sd(sc_score_sub, na.rm = TRUE),
            ari_main = sd(ari_score_main, na.rm = TRUE),
            ari_sub = sd(ari_score_sub, na.rm = TRUE),
            MSE_main = sd(cdist_main, na.rm = TRUE),
            MSE_sub = sd(cdist_sub, na.rm = TRUE), .groups = "drop")
# comresult_std2 <- comresult_std %>% group_by(tag) %>%
#   summarise(tag = min(tag),
#             per_main = mean(per_main, na.rm = TRUE),
#             per_sub = mean(per_sub, na.rm = TRUE),
#             sc_main = mean(sc_main, na.rm = TRUE),
...
df_report2 <- df_report %>%
  ...
  summarise(per_main = sum(main_grn == 2)/n(),
            ...
            MSE_main = mean(cdist_main, na.rm = T),
            MSE_sub = mean(cdist_sub, na.rm = T))
df_report2$tag <- "proposed"
df_report2 <- df_report2 %>%
  select(colnames(comresult_mean))
df_plot <- rbind(df_report2, comresult_mean) %>% select(-'dt_seed')
```

```{r}
library(reshape2)
df_plot_long <- melt(df_plot, id.vars = "tag")
df_plot_long$tag <- factor(df_plot_long$tag, levels = c('proposed', 'FMR2', 'resiclust2', 'respclust2', 'FMR1', 'resiclust1', 'respclust1'))
```
