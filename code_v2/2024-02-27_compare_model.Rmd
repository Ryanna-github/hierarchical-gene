...
FMR2_comp <- function(q_c_seed, minprior_value = 0.2, tag = "FMR2"){
  ...
  for(k_main in 1:est_main_grn){
    ...
    rownames(coef_alpha_est_k_main) <- NULL
    coef_full_est <- cbind(coef_full_est,
                           rbind(matrix(rep(coef_beta_est[,k_main], est_sub_grn_k_main), 
                                        ncol = est_sub_grn_k_main),
                                 coef_alpha_est_k_main))
...
respclust1_comp <- function(q_c_seed, tag = "respclust1"){
  ...
  cdist_main <- tryCatch({ coef_dist(coef_full_est[1:p,], coef$coef_alpha) }, error = function(err) {NaN})
  cdist_sub <- tryCatch({ coef_dist(coef_full_est[(p+1):(p+q),], coef$coef_alpha) }, error = function(err) {NaN})
  sc_score_sub <- sc(ci_est_sub, ci_sim_sub)
  ari_score_sub <- ari(ci_est_sub, ci_sim_sub)
  ci_matrix <- matrix(0, nrow = n, ncol = est_sub_grn)
...
respclust2_comp <- function(q_c_seed, tag = "respclust2"){
  ...
  idx_chosen <- which.max(rowMeans(df_K_set[,5:6]))
  ...
  return(list(dt_seed = dt_seed,
              q_c_seed = q_c_seed,
              tag = tag,
              cdist_main = df_K_set$cdist_main[idx_chosen],
              cdist_sub = df_K_set$cdist_sub[idx_chosen],
              # coef_full_ori = coef_full_est,
              sc_score_main = df_K_set$sc_score_main[idx_chosen],
              sc_score_sub = df_K_set$sc_score_sub[idx_chosen],
              ari_score_main = df_K_set$ari_score_main[idx_chosen],
...
oracle_comp <- function(q_c_seed, iter_type = 'stop', iter_max = 200, 
  ...
  coef_full_init <- matrix(0, nrow = p+q, ncol = K_up_true)
...
q_c_seed <- 1
iter_max <- 200
l2_seq <- c(0,1,1.5,2,4)
l3_seq <- c(0,1,1.5,2,4)
